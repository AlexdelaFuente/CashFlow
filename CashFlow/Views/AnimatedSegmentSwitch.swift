...
@IBDesignable open class AnimatedSegmentSwitch: UIControl {
    ...
    @objc open var selectedIndex: Int = 0 {
        didSet {
            ...
            sendActions(for: .valueChanged) // Enviar la acción para valueChanged
        }
    }
    ...
    override open func layoutSubviews() {
        ...
        selectFrame.origin.x = thumbInset + (newWidth * CGFloat(selectedIndex))
    ...
    fileprivate func nearestIndexAtLocation(location: CGPoint) -> Int? {
        ...
        for (index, item) in labels.enumerated() {
            let distance = abs(location.x - item.center.x)
            calculatedDistances.append(distance)
        }
        guard let minDistance = calculatedDistances.min() else {
            return nil
        }
        return calculatedDistances.firstIndex(of: minDistance)
    }
